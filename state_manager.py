...
Version: 5.1.39 (Corrected - SimConnect Mock Support)
Purpose: Manages application state transitions and conversation flow
Last Updated: March 28, 2025
Author: Your Name

Changes:
- Added mock SimConnect server support
...
class StateManager:
    ...
    def set_active_api(self, api_provider):
        ...
        if api_lower not in ai_manager.providers:
            ...
            if audio_processor:
                audio_processor.speak(f"Available: {', '.join(name.capitalize() for name in available)}")
            return False
        ...
        if audio_processor and self.current_state in [AppState.ACTIVE, AppState.WAITING]:
            time.sleep(0.3)
    ...
    def handle_command(self, command):
        ...
        if current_st == AppState.STANDBY:
            ...
            # --- Other command routing ---
                ...
                threading.Thread(target=self._handle_where_am_i_with_tour, daemon=True).start()
                return True
            elif self._is_navigation_query(cmd_lower):
                if audio_processor:
                    audio_processor.stop_continuous_listening()
                self.change_state(AppState.PROCESSING, "Nav request")
                threading.Thread(target=self._handle_navigation_request, args=(command,), daemon=True).start()
                return True
            elif self._is_tour_request(cmd_lower):
                if audio_processor:
                    audio_processor.stop_continuous_listening()
                self.change_state(AppState.PROCESSING, "Tour request")
                threading.Thread(target=self._handle_tour_request, args=(command,), daemon=True).start()
                return True
                ...
                threading.Thread(target=self._handle_general_question, args=(command,), daemon=True).start()
                return True
        else: # Busy state
    ...
    def _handle_where_am_i_with_tour(self):
        ...
        try:
            ...
            msg = [{"role":"system","content":prompt}, {"role":"user","content":"Loc info."}]
    ...
    def _handle_general_question(self, question):
        ...
        try:
            ...
            is_redundant = bool(msgs and msgs[-1].get("role") == "user" and 
                               msgs[-1].get("content") == question)

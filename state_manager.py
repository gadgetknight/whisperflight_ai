...
if sim_server is not None:
    ...
    # Define placeholders if required modules couldn't be imported
    navigation_manager = None
    geo_utils = None
    efb = None
...
class StateManager:
        ...
        with self.state_lock:
            ...
            if efb and hasattr(efb, "update_status"):
                try:
                    ...
                    self.logger.error(f"EFB status update error: {e}")

            reason_text = f" - Reason: {reason}" if reason else ""
            self.logger.info(
                f"State change: {self.previous_state.name} -> {self.current_state.name}{reason_text}"
            )

            if new_state == AppState.ACTIVE:
                print("STATUS: Ready.")
            elif new_state == AppState.WAITING:
                print("STATUS: Waiting follow-up.")
            elif new_state == AppState.PROCESSING:
                print("STATUS: Processing...")
            elif new_state == AppState.STANDBY:
                print("STATUS: Standby.")
            elif new_state == AppState.NAVIGATION:
    ...
    def set_active_api(self, api_provider):
        ...
        if not ai_manager or not hasattr(ai_manager, "providers"):
            self.logger.error("AIManager N/A.")
            return False

        api_lower = api_provider.lower()
        if api_lower not in ai_manager.providers:
            available = list(ai_manager.providers.keys())
            self.logger.warning(f"Unknown API: {api_provider}. Avail: {available}")
            if audio_processor:
                audio_processor.speak(
                    f"Available: {', '.join(name.capitalize() for name in available)}"
                )
            return False

        if self.active_api != api_lower:
            self.active_api = api_lower
            self.logger.info(f"Active API set: {self.active_api}")
            if efb and hasattr(efb, "update_api"):
                try:
                    efb.update_api(self.active_api.capitalize())
                except Exception as e:
                    self.logger.error(f"EFB API update error: {e}")
        else:
            self.logger.info(f"API already: {self.active_api}")

        if audio_processor and self.current_state in [
            AppState.ACTIVE,
            AppState.WAITING,
        ]:
            time.sleep(0.3)
    ...
    def add_to_conversation(self, role, content):
        ...
        if efb and hasattr(efb, "add_message"):
            try:
                ...
                self.logger.error(f"EFB msg error: {e}")
    ...
    def clear_conversation(self):
        ...
        if efb and hasattr(efb, "clear_conversation"):
            try:
                ...
                self.logger.error(f"EFB clear error: {e}")
    ...
    def _clear_audio_queue(self):
        if audio_processor and hasattr(audio_processor, "audio_queue"):
            ...
            while not audio_processor.audio_queue.empty():
                try:
                    audio_processor.audio_queue.get_nowait()
                    count += 1
                except queue.Empty:
                    break
                except Exception as e:
                    self.logger.error(f"Queue clear err: {e}")
                    break
            if count > 0:
                self.logger.info(f"Cleared {count} items from queue.")
        else:
            self.logger.warning("Audio queue N/A.")
    ...
    def handle_wake_word(self, wake_word):
        ...
        if any(v in wake_lower for v in fuzzy) and len(wake_lower.split()) <= 3:
            if audio_processor:
                audio_processor.speak("Say Sky Tour clearly.")
            return True
        ...
        if any(p in wake_lower for p in reactivate):
            ...
            if "where am i" not in wake_lower:
                if audio_processor:
                    if "question" in wake_lower:
                        audio_processor.speak("Okay, what's your question?")
                    else:
                        audio_processor.speak("Whisper AI ready.")
            return True
    ...
    def handle_command(self, command):
        ...
        junk_phrases = [
            ...
            "you're hovering near",
            "you're soaring above",
            "in the 08036 zip code area",
        ...
        is_junk = False
        if not command_strip or len(command_strip) < 3:
            is_junk = True
        elif cmd_lower_strip in junk_exact:
            is_junk = True
        # Check if the transcription *starts with* common junk phrases
        elif any(cmd_lower_strip.startswith(phrase) for phrase in junk_phrases):
            is_junk = True
        ...
        if current_st == AppState.STANDBY:
            ...
            if cmd_lower != "question" and not any(
                cmd_lower.startswith(neg)
                for neg in ["no", "nope", "stop", "that's all", "nothing else"]
            ):
            ...
            if current_st == AppState.WAITING and any(
                cmd_lower.startswith(neg) for neg in negative_responses
            ):
                self.logger.info("User indicated stop/no more questions.")
                if audio_processor:
                    audio_processor.speak("Okay, going to standby.")
                if audio_processor:
            ...
            # --- Other command routing ---
                ...
                provider = None
                if "grok" in cmd_lower:
                    provider = "grok"
                elif "open" in cmd_lower:
                    provider = "openai"
                if provider:
                    self.set_active_api(provider)
                else:
                    if audio_processor:
                        audio_processor.speak("Options: Grok, OpenAI.")
                if audio_processor and self.current_state in [
                    AppState.ACTIVE,
                    AppState.WAITING,
                ]:
                ...
                if audio_processor:
                    audio_processor.stop_continuous_listening()
                self.change_state(AppState.PROCESSING, "Where am I cmd")
                threading.Thread(
                    target=self._handle_where_am_i_with_tour, daemon=True
                ).start()
                return True
                ...
                threading.Thread(
                    target=self._handle_navigation_request, args=(command,), daemon=True
                ).start()
                return True
                ...
                threading.Thread(
                    target=self._handle_tour_request, args=(command,), daemon=True
                ).start()
                return True
                ...
                if self.last_destination:
                    if audio_processor:
                        audio_processor.stop_continuous_listening()
                    self.change_state(AppState.PROCESSING, "Setup notify")
                    success = self._setup_arrival_notification()
                    if success:
                        msg = f"Tracking: {self.last_destination}."
                        next_state = AppState.NAVIGATION
                    else:
                        msg = "Couldn't setup tracking."
                        next_state = AppState.ACTIVE
                    if audio_processor:
                        audio_processor.speak(msg)
                    self.change_state(next_state, "Notify setup done")
                    if audio_processor and self.current_state != AppState.STANDBY:
                        time.sleep(0.3)
                        audio_processor.start_continuous_listening()
                else:
                    if audio_processor:
                        audio_processor.speak("No destination.")
                ...
                if cmd_lower == "question":
                    if audio_processor:
                        audio_processor.speak("Okay, what is your question?")
                ...
                if audio_processor:
                    audio_processor.stop_continuous_listening()
                self.change_state(AppState.PROCESSING, "General question")
                threading.Thread(
                    target=self._handle_general_question, args=(command,), daemon=True
                ).start()
                return True
        else:  # Busy state
            self.logger.warning(
                f"Command '{command}' ignored in state {current_st.name}."
            )
            return False
    ...
    def _handle_where_am_i_with_tour(self):
        ...
        try:
            ...
            msg = [
                ...
                {"role": "user", "content": "Loc info."},
            ]
            hist = list(self.conversation_context)
            msgs = [msg[0]] + hist[-(self.context_length - 2) :] + [msg[1]]

            if len(msgs) > self.context_length:
                msgs = [msgs[0]] + msgs[-(self.context_length - 1) :]

            response = ai_manager.generate_response(msgs, self.active_api)
            error_indicators = getattr(ai_manager, "error_indicators", [])
            is_err = any(response.startswith(i) for i in error_indicators)
            ...
            if speak_ok and not is_err:
                ...
                if audio_processor:
                    audio_processor.speak("More questions?")
                next_state = AppState.WAITING
                reason = "Loc provided"
            elif is_err:
                reason = "AI error loc"
            else:
                self.logger.error("TTS fail.")
                reason = "TTS fail"
        except Exception as e:
            self.logger.error(f"Err where_am_i: {e}", exc_info=True)
            if audio_processor:
                audio_processor.speak("Error getting location.")
            reason = "Error loc proc"
        finally:
    ...
    def _handle_navigation_request(self, query):
        ...
        try:
            ...
            if audio_processor:
                audio_processor.speak("Calculating...")
            else:
                raise RuntimeError("Audio processor N/A.")
            ...
            error_flags = ["I couldn't find", "I encountered"]
            is_err = any(response.startswith(i) for i in error_flags)
            ...
            if speak_ok and not is_err:
                self.last_destination = dir_info["destination_name"]
                self.last_dest_lat = dir_info["latitude"]
                self.last_dest_lon = dir_info["longitude"]
                self.add_to_conversation("assistant", response)
                if audio_processor:
                    audio_processor.speak("Notify on arrival?")
                next_state = AppState.WAITING
                reason = "Nav provided"
            elif is_err:
                reason = "Nav lookup err"
            else:
                self.logger.error("TTS fail.")
                reason = "TTS fail"
        except Exception as e:
            self.logger.error(f"Err nav req: {e}", exc_info=True)
            if audio_processor:
                audio_processor.speak("Error calculating.")
            reason = "Nav proc error"
        finally:
            if next_state in [AppState.ACTIVE, AppState.WAITING]:
                if audio_processor:
                    time.sleep(0.3)
    ...
    def _setup_arrival_notification(self):
        ...
        if (
            not self.last_destination
            or self.last_dest_lat is None
            or self.last_dest_lon is None
        ):
            return False
        ...
        try:
            ...
            if success:
                self.logger.info(f"Track started: {self.last_destination}")
            else:
                self.logger.warning(f"Failed track start: {self.last_destination}")

            return success
        except Exception as e:
            self.logger.error(f"Err setup arrival: {e}", exc_info=True)
            return False

    def _handle_tour_request(self, query):
        next_state = AppState.ACTIVE
        reason = "Tour proc error"
        try:
            ...
            user_q = query if query != "tour request" else "Interesting things?"
            user_msg = {"role": "user", "content": user_q}

            msgs = self.get_conversation_context()
            msgs = [sys_msg] + msgs[-(self.context_length - 2) :] + [user_msg]
            ...
            if speak_ok and not is_err:
                ...
                if audio_processor:
                    audio_processor.speak("More questions?")
                next_state = AppState.WAITING
                reason = "Tour provided"
            elif is_err:
                reason = "AI error tour"
            else:
                self.logger.error("TTS fail.")
                reason = "TTS fail"
        except Exception as e:
            self.logger.error(f"Err tour req: {e}", exc_info=True)
            if audio_processor:
                audio_processor.speak("Error preparing tour.")
            reason = "Tour proc error"
        finally:
            if next_state in [AppState.ACTIVE, AppState.WAITING]:
                if audio_processor:
    ...
    def _handle_general_question(self, question):
        ...
        try:
            ...
            hist = list(self.conversation_context)
            recent = hist[-(self.context_length - 1) :]
            msgs.extend(recent)

            is_redundant = bool(
                msgs
                and msgs[-1].get("role") == "user"
                and msgs[-1].get("content") == question
            )
            ...
            if audio_processor:
                audio_processor.speak("Trouble processing.")
            reason = "Question error"
        finally:
            if next_state in [AppState.ACTIVE, AppState.WAITING]:
                if audio_processor:
    ...
    def on_arrival(self, destination):
        with self.state_lock:
            if self.current_state not in [AppState.STANDBY, AppState.ERROR]:
                if audio_processor:
                    audio_processor.speak(f"Arrived: {destination}!")
                self.navigation_active = False
                self.change_state(AppState.ACTIVE, f"Arrived: {destination}")
                if audio_processor:
                    time.sleep(0.3)
                    audio_processor.start_continuous_listening()
            else:
    ...
    def on_one_minute_away(self, destination, distance):
        with self.state_lock:
            if self.current_state == AppState.NAVIGATION and self.navigation_active:
                if audio_processor:
                    audio_processor.speak(f"Approaching {destination}, 1 min.")
    ...
    def on_off_course(self, current_heading, target_heading, difference):
        with self.state_lock:
            if self.current_state == AppState.NAVIGATION and self.navigation_active:
                ...
                if audio_processor:
                    audio_processor.speak(
                        f"Correction: Turn {correction} to {target_heading:.0f} for {self.last_destination}."
                    )

    def on_update(self, distance, heading, eta):
        self.logger.debug(
            f"Nav update: Dist={distance:.1f}nm, Hdg={heading:.0f}, ETA={eta:.1f}min"
        )
        pass

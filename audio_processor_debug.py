...
# ... existing code ...
debug_log("No wake phrase or command detected in this transcription", "INFO")
# Fixed: Make sure lower_text variable is defined properly
...
class SpeechToText(ABC):
    ...
    def record_audio(self):
        debug_log(
            f"Recording audio started for {self.recording_seconds} seconds", "INFO"
        )
        ...
        for i in range(p.get_device_count()):
            ...
            devices_info.append(
                f"Device {i}: {info['name']} (Inputs: {info['maxInputChannels']})"
            )
        ...
        for attempt in range(max_retries):
            try:
                debug_log(
                    f"Opening audio stream (attempt {attempt+1}/{max_retries})", "INFO"
                )
                stream = p.open(
                    format=self.format,
                ...
                max_level = 0
                for i in range(
                    int(self.sample_rate / self.chunk_size * self.recording_seconds)
                ):
                    ...
                    audio_level = (
                        np.abs(audio_data).mean() / 32767.0
                    )  # Normalize to 0-1
                    ...
                    if i % 10 == 0:
                        ...
                        debug_log(
                            f"Audio level: {audio_level:.4f} [{level_indicator}]",
                            "DEBUG",
                        )
                ...
                debug_log(
                    f"Recording finished. Max audio level: {max_level:.4f}", "INFO"
                )
                ...
                debug_log(
                    f"Error recording audio (attempt {attempt + 1}/{max_retries}): {e}",
                    "ERROR",
                )
    ...
    def get_input(self):
        ...
        try:
            ...
            try:
                ...
                debug_log(
                    f"Failed to remove temporary file {audio_file}: {e}", "WARNING"
                )
...
class WhisperSTT(SpeechToText):
    def __init__(self):
        ...
        self.noise_threshold = config.getfloat(
            "Speech", "noise_threshold", 0.05
        )  # Lowered to 0.05
        debug_log(
            f"WhisperSTT initialized with model={self.model_name}, noise_threshold={self.noise_threshold}",
            "INFO",
        )
        self._load_model()
    ...
    def _load_model(self):
        ...
        try:
            ...
            debug_log(
                f"Whisper {self.model_name} model loaded successfully in {load_time:.2f} seconds",
                "INFO",
            )
        except Exception as e:
    ...
    def transcribe(self, audio_data):
        ...
        try:
            debug_log(
                f"Starting Whisper transcription for audio file: {audio_data}", "INFO"
            )
            ...
            if file_size < 100:  # Suspiciously small file
                debug_log(
                    "Audio file is suspiciously small, possibly empty recording",
                    "WARNING",
                )
            ...
            debug_log(
                f"Transcription completed in {transcription_time:.2f} seconds", "INFO"
            )
            ...
            for variation in wake_variations:
                if variation in lower_text:
                    debug_log(
                        f"WAKE WORD MATCH FOUND! '{variation}' in '{lower_text}'",
                        "INFO",
                    )
                    print(
            ...
            if any(
                word in lower_text
                for word in ["grok", "growth", "rock", "crock", "roc"]
            ):
            ...
            if any(
                word in lower_text
                for word in ["open ai", "openai", "open eye", "open a", "gpt"]
            ):
                debug_log(f"OpenAI command detected: '{lower_text}'", "INFO")
                return "use openai"

            if "philadelphia" in lower_text and any(
                word in lower_text for word in ["airport", "international"]
            ):
                debug_log(
                    f"Philadelphia airport command detected: '{lower_text}'", "INFO"
                )
...
class GoogleSTT(SpeechToText):
    ...
    def transcribe(self, audio_data):
        ...
        try:
            debug_log(
                f"Starting Google STT transcription for audio file: {audio_data}",
                "INFO",
            )
    ...
    def speak(self, text):
        ...
        try:
            ...
            if not pygame.mixer.get_init():
                debug_log(
                    "PyGame mixer not initialized, attempting to initialize", "WARNING"
                )
                pygame.mixer.init()
            ...
            try:
                ...
                debug_log(
                    f"Failed to remove temporary file {temp_filename}: {e}", "WARNING"
                )
    ...
    def synthesize(self, text, output_file):
        ...
        try:
            ...
            for attempt in range(max_retries):
                try:
                    debug_log(
                        f"ElevenLabs API request (attempt {attempt+1}/{max_retries})",
                        "INFO",
                    )
                    start_time = time.time()
                    ...
                    debug_log(
                        f"ElevenLabs API response received in {response_time:.2f} seconds",
                        "INFO",
                    )
                    ...
                    debug_log(
                        f"Audio file size: {os.path.getsize(output_file)} bytes",
                        "DEBUG",
                    )
                    return True
                except Exception as e:
                    debug_log(
                        f"Error in ElevenLabs synthesis attempt {attempt + 1}/{max_retries}: {str(e)}",
                        "ERROR",
                    )
...
class GoogleTTS(TextToSpeech):
    ...
    def synthesize(self, text, output_file):
        ...
        try:
            ...
            debug_log(
                f"Google TTS synthesis successful in {synthesis_time:.2f} seconds",
                "INFO",
            )
...
class AudioProcessor:
    ...
    def _create_stt_provider(self):
        ...
        if provider_name == "whisper" and WHISPER_AVAILABLE:
            ...
            debug_log(
                f"STT provider '{provider_name}' unavailable, using Whisper as fallback",
                "WARNING",
            )
            return WhisperSTT()
        elif GOOGLE_STT_AVAILABLE:
            debug_log(
                f"STT provider '{provider_name}' unavailable, using Google as fallback",
                "WARNING",
            )
            return GoogleSTT()
    ...
    def _create_tts_provider(self):
        ...
        if provider_name == "elevenlabs" and ELEVENLABS_AVAILABLE:
            ...
            debug_log(
                f"TTS provider '{provider_name}' unavailable, using ElevenLabs as fallback",
                "WARNING",
            )
            return ElevenLabsTTS()
        elif GTTS_AVAILABLE:
            debug_log(
                f"TTS provider '{provider_name}' unavailable, using Google as fallback",
                "WARNING",
            )
            return GoogleTTS()
        ...
        if sound_effect and sound_effect in self.sound_effects:
            ...
            try:
                ...
                if os.path.exists(sound_file):
                    ...
                    if not pygame.mixer.get_init():
                        debug_log(
                            "PyGame mixer not initialized for sound effect, initializing",
                            "WARNING",
                        )
                        pygame.mixer.init()
...
activate_file = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "Sky_Tour_Activated.mp3"
)
deactivate_file = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "Sky_Tour_Deactivated.mp3"
)
...
debug_log(
    f"Noise Threshold: {config.getfloat('Speech', 'noise_threshold', 0.05)}", "INFO"
)
debug_log(
    f"Recording Seconds: {config.getfloat('Speech', 'recording_seconds', 5)}", "INFO"
)
...
def run_audio_diagnostics():
    ...
    if audio_processor.stt_provider:
        debug_log(
            f"STT provider type: {type(audio_processor.stt_provider).__name__}", "INFO"
        )
    ...
    if audio_processor.tts_provider:
        debug_log(
            f"TTS provider type: {type(audio_processor.tts_provider).__name__}", "INFO"
        )
    else:
        debug_log("No TTS provider available", "ERROR")

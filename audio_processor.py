...
Version: 5.1.17 (Corrected - Robust Listen Control + STT Debug)
Purpose: Handles speech recognition and text-to-speech with multiple providers
Last Updated: March 28, 2025
Author: Your Name

Changes:
- Added print statement in _listen_worker to show all raw transcriptions.
- Made start/stop_continuous_listening more robust with thread checks/locks.
- Removed state check from TextToSpeech.speak finally block (handled by StateManager).
"""
        ...
        try:
            ...
            try:
             ...
             if 'temp_fn' in locals() and temp_fn and os.path.exists(temp_fn) and ('wf' not in locals() or not wf): 
                  try: os.remove(temp_fn); self.logger.debug("Cleaned failed temp WAV.")
                  except Exception as e: self.logger.warning(f"Failed cleanup temp WAV: {e}")
             elif 'temp_fn' in locals() and temp_fn and not os.path.exists(temp_fn): pass
...
class WhisperSTT(SpeechToText):
    ...
    def transcribe(self, audio_data):
        ...
        try:
            ...
            if text.startswith("1.0.1.1"): self.logger.warning("Invalid numeric transcription."); return ""
            return text 
...
class TextToSpeech(ABC):
    ...
    def speak(self, text, audio_processor_instance): 
        ...
        if audio_processor_instance: audio_processor_instance.stop_continuous_listening()
        ...
        try:
            ...
            if not synthesis_ok: self.logger.error(f"Synthesis failed: '{text[:50]}...'"); speak_success = False
                ...
                except pygame.error as unload_err: self.logger.warning(f"Error unloading music: {unload_err}")
                speak_success = True
...
class AudioProcessor:
    def __init__(self):
        ...
        self.audio_queue = queue.Queue(); self.is_listening = False; self.listen_thread = None 
    ...
    def start_continuous_listening(self):
        with self.listen_lock: 
             if self.is_listening: self.logger.debug("Start ignored, already listening."); return
    ...
    def stop_continuous_listening(self):
        with self.listen_lock: self._stop_listen_thread_internal() 

    def _stop_listen_thread_internal(self):
        # Internal method assumes lock is held
        if not self.is_listening and not (hasattr(self, 'listen_thread') and self.listen_thread and self.listen_thread.is_alive()): return
        ...
        if thread_to_join and thread_to_join.is_alive() and threading.current_thread() != thread_to_join: 
             ...
             try: 
                 ...
                 if thread_to_join.is_alive(): self.logger.warning("Listen thread failed to join!")
                 else: self.logger.info("Stopped continuous listening thread.")
             except Exception as e: self.logger.error(f"Error joining listen thread: {e}")
        elif thread_to_join == threading.current_thread(): self.logger.warning("Stop called from listen thread itself.")
        else: self.logger.info("Stopped listening signal sent."); self.listen_thread = None # Ensure it's cleared if already stopped
        
        # Clear queue after stopping
        if hasattr(self, 'audio_queue'):
             count = 0;
             while not self.audio_queue.empty():
                 try: self.audio_queue.get_nowait(); count += 1
                 except queue.Empty: break
                 except Exception: break 
             if count > 0: self.logger.debug(f"Cleared {count} items from queue on stop.")
            ...
            try:
                 ...
                 if self.is_listening and text: 
                     ...
                     if len(text.strip()) > 1 : # Basic check before queueing
                          ...
                          self.logger.debug(f"Ignoring very short STT result before queueing: '{text}'")

                 # Yield control briefly
                 time.sleep(0.1) 
            except Exception as e: 
                self.logger.error(f"Listen worker error: {e}", exc_info=True); 
                if self.is_listening: time.sleep(1) # Avoid spamming errors
        self.logger.info("Listen worker thread finished.")

    def get_next_command(self, block=True, timeout=None):
        try: command = self.audio_queue.get(block=block, timeout=timeout); self.logger.info(f"Queue retrieve: '{command}'"); return command
        except queue.Empty: 
